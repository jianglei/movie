...
if(!!~$.inArray(a.param.name,["list","platform"])){$("div.sel_wrap,div.sel_title").hide();}if(a.param.name=="ad"){$(".output_csv").show();}if(b[a.param.name]){$("div.sel_title1").text(b[a.param.name]);
}$(".sel_wrap").click(function(){$(this).find(".pop_menu").slideToggle(300,function(){$(".con",this).next().toggleClass("arrow_up");});});$(".sel_wrap1").delegate("li","click",function(){$(".sel_wrap1 .con").text($(this).text()).attr("title",$(this).text()).attr("a_id",$(this).find("a").attr("id"));
a.currentToken=a.getToken();a.getPostData();a.loadData($(this).find("a").attr("id"),a.param.data);});if(a.param.name=="platform"){$("div.sel_wrap3,div.sel_title3").show();
$(".sel_wrap3").delegate("li","click",function(){$(".sel_wrap3 .con").text($(this).text()).attr("title",$(this).text()).attr("platform",$(this).find("a").attr("platform"));
a.param.data.platform=$(".sel_wrap3 .con").attr("platform");a.currentToken=a.getToken();a.getPostData();a.loadData(null,a.param.data);});}if(!!~$.inArray(a.param.name,["app","adslot","list"])){$("div.sel_wrap2,div.sel_title2").show();
...
}a.param.data.period=a.period=$(this).attr("period");$(".panel_choice_time").hide();$(this).addClass("current").siblings().removeClass("current");a.getPostData();
if(a.period!="today"&&a.period!="yesterday"){$(".panel_chart").show();}else{$(".panel_dashboard .item span").text("-");$(".panel_chart").hide();}a.currentToken=a.getToken();
a.loadData(a.param.dataUrl=="/report/total"?"":$(".sel_wrap1 .con").attr("a_id"),a.param.data);return false;});$(".btn_choice_time").click(function(){$(this).addClass("current").siblings().removeClass("current");
$(this).parent().toggleClass("toggle_choice_time");$(".panel_choice_time").toggle();$(".output_csv").toggleClass("cl");});$("#cus_start_time").datepicker({dateFormat:"yy-mm-dd",changeYear:true,changeMonth:true,monthNamesShort:["一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"],dayNames:["星期日","星期一","星期二","星期三","星期四","星期五","星期六"],dayNamesMin:["日","一","二","三","四","五","六"]});
...
$(".panel_choice_time button").click(function(){a.currentToken=a.getToken();delete a.period;a.getPostData();$(".panel_chart").show();a.loadData(a.param.dataUrl=="/report/total"?"":$(".sel_wrap1 .con").attr("a_id"),a.param.data);
});},dataFilter:{round:function(b){var a=b.toString().split(".")[1];if(a!==undefined&&a.length>2){return b.toFixed(2);}else{return b;}},getClass:function(){var a=new Date(this.data.date.replace(/-/g,"/")).getDay();
if(!!~$.inArray(a,[0,6])){return"weekday";}else{return"";}}},getToken:function(){return Math.random().toString(16).substring(2);},getPostData:function(){var a=this;
delete a.param.data.period;delete a.param.data.startDate;delete a.param.data.endDate;delete a.param.data.category;a.promotionOpen=false;a.param.data.category=$(".sel_wrap2 .con").attr("category");
if($("div.panel_choicelnk a.current").index()<6){a.param.data.period=a.period;}else{if($("#cus_start_time").val()!==""&&$("#cus_end_time").val()!==""){a.param.data.startDate=$("#cus_start_time").val();
a.param.data.endDate=$("#cus_end_time").val();}}if($(".sel_wrap2 .con").attr("category")=="0"){a.promotionOpen=true;}},tmplList:function(c){var b=this;
if(this.param.listTmpl.length>0){$(this.param.listTmpl).tmpl(c).appendTo(this.param.listContainer);var a=$(b.param.listContainer).find("li a").eq(0).text(),d=$(b.param.listContainer).find("li a").eq(0).attr("id");
$(".sel_wrap1 span.con").text(a).attr("title",a).attr("a_id",d);this.loadData($(b.param.listContainer).find("li a").eq(0).attr("id"));}},tmplData:function(a){if(this.param.dataTmpl.length>0){$(this.param.dataContainer).find("tr:gt(1)").remove();
...
$("tr.promotionReportTitle").hide();$(this.param.dataTmpl).tmpl(a.reportList,this.dataFilter).appendTo(this.param.dataContainer);if(this.period!="today"&&this.period!="yesterday"){this.drawGraph(a.reportList);
}this.dashBoard(a.summary);}}},loadList:function(){var a=this;a.param.data.rnd=Math.random();$.ajax({type:a.param.type,url:a.param.listUrl,data:a.param.data,beforeSend:function(){$(a.param.listContainer).find("li").remove().parents("div.sel_wrap").find("span.con").text("加载中！");
},success:function(b,c){loginValidate(b);if(b[a.param.resultInterface]!==undefined&&b[a.param.resultInterface].result.length>0){a.tmplList(b[a.param.resultInterface].result);
...
$("#chart1").text("加载中！");},success:function(e,f){loginValidate(e);if(a!=b.currentToken){return;}if(e.status=="failed"){$(b.param.dataContainer).find("td.wait").html("加载失败！");
$("#chart1").text("加载失败！");return false;}if(e&&e.reportList!==null&&e.reportList.length>0){b.tmplData(e);}else{$(b.param.dataContainer).find("td.wait").html("没有记录！");
