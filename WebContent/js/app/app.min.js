...
}});loadList(1);});});$(window).resize(function(){adjustMsg();});});function clearList(){$("#tb_list tr").each(function(a){if($(this).attr("class")!="tit"){$(this).remove();
...
c.find(".tb_platform").html(a);},1000);}function initMsg(e,g,f){$(".err").remove();if(e.name&&e.name!==""){$(".msg_content_top .title span").eq(0).text("编辑应用");
}else{$(".msg_content_top .title span").eq(0).text("新建应用");}$("input[name='name']").val(e.name);$(".ui_radio").removeClass("ui_radio_checked");if(e.platform&&e.platform=="android"){$(".android").parent().addClass("ui_radio_checked");
$("input[name='platform']").val(e.platform);}else{if(e.platform&&e.platform=="iOS"){$(".ios").parent().addClass("ui_radio_checked");$("input[name='platform']").val(e.platform);
}}$("textarea[name='description']").val(e.description?e.description:"");var d=$(".msg_app");var a=$(window).height();var c=$(document).width();$.blockUI({css:{color:"#cccccc",border:"0",width:"818px",left:c/2-(d.width()/2),top:a/2-(d.height()/2),background:"none",padding:"0px"},message:$(".msg_app")});
...
$(this).addClass("ui_radio_checked");});function b(){if(verify_null($("input[name='name']"),"应用名称不能为空！")&&verify_null($("input[name='platform']"),"平台不能为空！",true)){var j=this;
$(j).unbind("click");var k=$("input[name='name']").val();var h=$("input[name='platform']").val();var l=$("textarea[name='description']").val();$.ajax({type:"get",url:"/app/save?name="+k+"&platform="+h+"&description="+l+(f?("&id="+f):""),dataType:"json",data:"rnd="+Math.random(),beforeSend:function(m){},success:function(m,n){if(m.status=="ok"){g();
}},complete:function(m,n){},error:function(){alert("保存失败!");$(".btn_save").unbind("click").click(function(){b.apply(this);});}});}}$(".btn_save").unbind("click").click(function(){b.apply(this);
...
updateItem(c.parent().parent(),h,g);$.unblockUI({onUnblock:function(){appDialogId="";}},d);});},complete:function(e,f){},error:function(){}});}function showMask(){var b=$(".panel_table").css("height").replace("px","");
var a=b/2-58;$(".loading").css("display","block").css("padding-top",a+"px").css("height",(b-a)+"px");}function hideMask(){$(".loading").css("display","none");
}function adjustMsg(){var a=$(".msg_app").is(":visible")?$(".msg_app"):$(".msg_ad");$("body>.blockMsg").stop(true).animate({top:$(window).height()/2-(a.height()/2)+"px"},300);
}