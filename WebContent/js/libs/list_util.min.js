var url_status,uploadFileHd,uploadFileWait;function pageAnchorsGenerate(e,b,d,f){function c(k,m){var n="",g,l,j=3;g=m-j;l=m+j;if(g<1){l=l-g+1;}if(l>k){g=g-l+k;
}for(var h=0;h<k;h++){if(h===0||h==k-1||(h+1>=g&&h+1<=l)){n+='<a class="'+(h+1==m?"current":"")+'" p="'+(h+1)+'">'+(h+1)+"</a>";}else{if(h==k-2){if(l<k-2){n+='<a class="dot" p="'+(h+1)+'">...</a>';
}else{n+='<a  p="'+(h+1)+'">'+(h+1)+"</a>";}}else{if(h==1){if(g>3){n+='<a class="dot" p="'+(h+1)+'">...</a>';}else{n+='<a  p="'+(h+1)+'">'+(h+1)+"</a>";
}}}}}return n;}var a=c(e,b);d=(d&&d!=="")?d:"#pager";$(d).html("").append(a).find("a:not(.current)").unbind("click").click(function(){f($(this).attr("p"),url_status);
});$("#loading_row_init").remove();}function loginValidate(a){if(a.status=="login"){location.pathname="/login";return false;}}function adjustMsg(){var a=$(window).height()/2-($(".blockMsg .msgForm").height()/2);
a=a<0?0:a;$(".blockMsg").css({top:a+"px"});}function popBox(d){var a=$(window).height(),b=$(document).width(),c=a/2-(d.height()/2);c=c<0?0:c;$.blockUI({css:{overflow:"hidden",height:0,position:"relative",color:"#cccccc",border:"0",width:"722px",left:b/2-(d.width()/2),top:c,background:"none",padding:"0px"},overlayCSS:{backgroundColor:" rgba(0,0,0,.5)",opacity:1,overflow:"auto"},allowBodyStretch:true,message:d,onBlock:function(){$(".blockOverlay").append($(".blockMsg"));
$("body").css({"overflow-y":"hidden"});$(".blockMsg").css({height:"auto",overflow:"visible"});}});}function set_date_param(b,a,c){$("#dateType").val(b);
$("#startDate").val(a);$("#endDate").val(c);}function checkAll(b,a){$("input[name="+a+"]").prop("checked",$(b).prop("checked"));}function checkAllAdSlot(b,a){if($(b).prop("checked")){if($("input[name="+a+"]:checked").length>0){$("input[name="+a+"]").not(":disabled").prop("checked",true);
...
$(d).prevAll('input[type="file"]').hide();}});}function upload(g,f,b,h,e,a){function d(){uploadFileWait=true;var i=$(e).attr("action");$(e+"_uploadType").val(f);
$(e+"_fileObject").val(g.id);$(e+"_feedback").val(b);$(e+"_callback").val(h);$(e).attr("target","uploadFrame");$(e).attr("action","/upload/uploadFile");
$(e).submit();$(e).attr("target","_self");$(e).attr("action",i);}if(g){if(verify_file($(g),$(g).attr("title"),$(g).attr("rule"),false,true)){$(g).parent().append('<img id="loadingStatus" src="/images/roller.gif" width="25" height="25"/>');
if(a&&a!="-"){var c=a.split("x");$(e+"_width").val(c[0]);$(e+"_height").val(c[1]);}if(!uploadFileWait){d();}else{uploadFileHd=setInterval(function(){if(uploadFileWait){return false;
...
},minLength:function(l,k){return l.length>=k?{pass:true}:{pass:false,msg:"长度不能小于 "+k};},fixLength:function(l,k){if($.type(k)=="string"){return l.length==k?{pass:true}:{pass:false,msg:"固定长度为 "+k};
}else{if($.type(k)=="array"){return !!~$.inArray(l.length,k)?{pass:true}:{pass:false,msg:"固定长度为 "+k};}}},url:function(l){var k=/^(http:|https:|telephone:|sdk:)\/\//ig;
console.log(k.test(l));if(k.test(l)&&$.trim(l.replace(k,""))===""){return{pass:false,msg:"请输入正确的url"};}else{return{pass:true};}}};var h=g?g:f,i={pass:true},a=f.val();
if(a.indexOf("<")>-1||a.indexOf(">")>-1||a.indexOf("script")>-1||a.indexOf("eval")>-1){i={pass:false,msg:"字符不符合规则"};}else{(function(){var l=["isNul","isNumberic","isDigit","maxV","minV","maxLength","minLength","fixLength","url"];
var k=0;$.each(d,function(m,n){if(n!="no"&&i.pass){i=j[l[k]](a,n);}k++;});})();}if(i.pass==false){f.parent().find(".err").remove();if(c){h.after("<div class='err'>"+i.msg+"</div>");
}else{h.after("<span class='err'>"+i.msg+"</span>");}f.focus();return false;}else{f.parent().find(".err").remove();return true;}}function verify_file(f,a,j,b,c){var g=new RegExp(j,"gi");
if(!g.test(f.val())){if(f.parent().find(".err").length==0){if(!c){if(b){f.after("<div class='err'>"+a+"</div>");}else{f.after("<span class='err'>"+a+"</span>");
}}}f.focus();return false;}else{var i=false;if(f[0].files){i=(f[0].files[0].size/1024>parseInt(f.attr("filesize")));}else{try{var d=new ActiveXObject("Scripting.FileSystemObject");
i=(d.GetFile(f.val()).size/1024>parseInt(f.attr("filesize")));}catch(h){}}if(i){if(!c){if(f.parent().find(".err").html()==null){if(b){f.after("<div class='err'>文件不得大于"+f.attr("filesize")+"K</div>");
...
}$(function(){initUISelect();umengRadioImitate();umengSelectIitate_withoutValue();$(window).resize(function(){adjustMsg();});$(".part_operation dd a").click(function(){change("record_ch",$(this).attr("action")+"/changes?","status",$(this).attr("status"));
...
$(this).addClass("show").find(".state_panel").width(a).show();}else{$(this).removeClass("show");$(this).find(".state_panel").hide();}});$("#container").delegate(".btn_change_state","click",function(b){b.stopPropagation();
var a=[{name:"正常",query:"normal"},{name:"应用暂停",query:"app_pause"},{name:"暂停",query:"pause"},{name:"到达预算",query:"limit"},{name:"订单暂停",query:"adorder_pause"},{name:"投放完成",query:"finish"},{name:"订单投放完成",query:"adorder_finish"},{name:"尚未投放",query:"ready"},{name:"订单尚未投放",query:"adorder_ready"},{name:"iOS",query:"iOS"},{name:"Android",query:"android"}],c="";
count=0;url_status="";$(this).closest(".state").find(".state_panel li").not(":last").find("input[type='checkbox']:checked").each(function(){count++;if($(this).siblings("label").hasClass("platform")){url_status+="&platform="+a[$(this).val()].query;
}else{url_status+=("&status="+($(this).attr("pausetype")?($(this).attr("pausetype")+"_"):"")+a[$(this).val()].query);}});if(count===0){c='<span class="none">未筛选</span>';
url_status="";}else{c='<span class="none">'+count+"个筛选条件 </span>";}$(this).closest(".state").find(".state_info").html(c);$(this).closest(".state").removeClass("show");
$(this).closest(".state").find(".state_panel").hide();loadList(1,url_status);});});