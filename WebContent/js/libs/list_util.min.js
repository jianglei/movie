...
},isDigit:function(k){return parseInt(k)==k?{pass:true}:{pass:false,msg:"输入整数"};},maxV:function(k,l){return parseInt(k)<=parseInt(l)?{pass:true}:{pass:false,msg:"不能大于 "+l};
...
}f.focus();return false;}else{f.parent().find(".err").remove();return true;}}function verify_file(f,a,j,b,c){var g=new RegExp(j,"g");if(!g.test(f.val())){if(f.parent().find(".err").length==0){if(!c){if(b){f.after("<div class='err'>"+a+"</div>");
