...
function loadList(b,a){$.ajax({type:"get",url:"ad/list?pageNo="+b+(a?a:""),dataType:"json",data:"rnd="+Math.random(),beforeSend:function(c){},success:function(g,e){var k=g;
var d=k.adPage.result;var h=k.adPage.totalPages;var c=k.adPage.pageNo;var j=d.length;var l=true;pageAnchorsGenerate(h,c,"",loadList);if(!k.adPage||k.adPage.result.length==0){$("#tb_list tr").not(":first").remove();
...
f+='<td class="tb_ad_slot_name" title="'+(d[i].ad_slot_name?d[i].ad_slot_name:"-")+'">'+(d[i].ad_slot_name?d[i].ad_slot_name:"-")+"</td>";f+="<td>"+(d[i].start_time?formatdate(new Date(d[i].start_time),"yyyy-MM-dd HH:mm"):"-")+"</td>";
f+="<td>"+(d[i].end_time?formatdate(new Date(d[i].end_time),"yyyy-MM-dd HH:mm"):"-")+"</td>";f+='<td style="position:relative;display:block;" id="td_level_'+d[i].id+'" style="text-align:left; padding:0 30px;" class="level">';
f+='<div class="pnl"><div style="text-align:center;">';f+='<span class="lvl i'+d[i].prior+" prior_img_"+d[i].id+'"></span></div>';f+='<ul style="padding:10px;">';
f+='<li><input type="radio" name="prior_'+d[i].id+'" id="prior1_'+d[i].id+'"';if(d[i].prior==1){f+='checked="checked" ';}f+='onclick="$(&quot;#prior_input_'+d[i].id+'&quot;).val(1);$(this).closest(&quot;.pnl&quot;).find(&quot;.lvl&quot;).removeClass(&quot;i1 i2 i3&quot;).addClass(&quot;i1&quot;)"/><label for="prior1_'+d[i].id+'">&nbsp;低级</label></li>';
...
});}function updatePrior(b,a){$.ajax({type:"GET",url:a,data:"rnd="+Math.random(),success:function(c){$("#td_level_"+b+".level .pnl").hide();$(".prior_img_"+b).removeClass("i1 i2 i3");
$(".prior_img_"+b).addClass("i"+$("#prior_input_"+b).val());}});}