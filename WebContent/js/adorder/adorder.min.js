...
}function adjustMsg(){var a=$(".msg_order").is(":visible")?$(".msg_order"):$(".msg_ad");$(".blockMsg").css("top",$(window).height()/2-(a.height()/2)+"px");
...
}}function showMsg(b,e){initMsg(b);var a=$(window).height();var c=$(document).width();var d=$(".msg_order");var a=$(window).height();var c=$(document).width();
$.blockUI({css:{color:"#cccccc",border:"0",width:"818px",left:c/2-(d.width()/2),top:a/2-(d.height()/2),background:"none",padding:"0px"},message:$(".msg_order")});
$(".close").unbind("click").click(function(){$.unblockUI({onUnblock:function(){adOrderDialogId="";}});});$(".btn_save_order").unbind("click").click(function(){if(verify_null($("input[name='name']"),"订单名称不能为空！")&&verify_null($("#datepicker_start_adorder"),"开始时间不能为空",false,$("#datepicker_end_adorder"))){var g=getADOrder();
var f="/adorder/save";g.rnd=Math.random(),$.ajax({type:"get",url:f,data:g,dataType:"json",beforeSend:function(h){},success:function(h,j){if(h.status=="ok"){}},complete:function(h,j){e();
},error:function(){alert("保存出错!");}});}});}function editAdOrder(d,a){$("input[name='id']").val(d);var c=$(a.target);var b=adOrderDialogId=getToken();$.ajax({type:"get",url:"/adorder/edit/"+d,dataType:"json",data:"rnd="+Math.random(),beforeSend:function(e){},success:function(f,g){if(b!=adOrderDialogId){return;
