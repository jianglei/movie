...
$("input[name='id']").val("");showMsg(new ADOrder(),function(){$.unblockUI({onUnblock:function(){adOrderDialogId="";}});loadList(1);});});$(window).resize(function(){adjustMsg();
});});function loadList(b,a){$.ajax({type:"get",url:"adorder/list?pageNo="+b+(a?a:""),dataType:"json",data:"rnd="+Math.random(),beforeSend:function(c){},success:function(g,d){var k=g.adOrderPage;
var f=k.result;var h=k.totalPages;var c=k.pageNo;var j=f.length;var l=true;clearList();pageAnchorsGenerate(h,c,"",loadList);if(!k||k.result.length===0){$("#tb_list tr").not(":first").remove();
...
}function adjustMsg(){var a=$(".msg_order").is(":visible")?$(".msg_order"):$(".msg_ad");$(".blockMsg").stop(true).animate({top:$(window).height()/2-(a.height()/2)+"px"},300);
...
}}function showMsg(b,f){initMsg(b);var a=$(window).height();var d=$(document).width();var e=$(".msg_order");var a=$(window).height();var d=$(document).width();
$.blockUI({css:{color:"#cccccc",border:"0",width:"818px",left:d/2-(e.width()/2),top:a/2-(e.height()/2),background:"none",padding:"0px"},message:$(".msg_order")});
$(".close").unbind("click").click(function(){$.unblockUI({onUnblock:function(){adOrderDialogId="";}});});function c(){if(verify_null($("input[name='name']"),"订单名称不能为空！")&&verify_null($("#datepicker_start_adorder"),"开始时间不能为空",false,$("#datepicker_end_adorder"))){var g=this;
$(g).unbind("click");var j=getADOrder();var h="/adorder/save";j.rnd=Math.random(),$.ajax({type:"get",url:h,data:j,dataType:"json",beforeSend:function(k){},success:function(k,l){if(k.status=="ok"){}},complete:function(k,l){f();
},error:function(){alert("保存出错!");$(g).unbind("click").click(function(){c.apply(g);});}});}}$(".btn_save_order").unbind("click").click(function(){c.apply(this);
});}function editAdOrder(d,a){$("input[name='id']").val(d);var c=$(a.target);var b=adOrderDialogId=getToken();$.ajax({type:"get",url:"/adorder/edit/"+d,dataType:"json",data:"rnd="+Math.random(),beforeSend:function(e){},success:function(f,g){if(b!=adOrderDialogId){return;
}var e=new ADOrder(f.adOrder.id,f.adOrder.name,f.adOrder.customer,f.adOrder.startTime,f.adOrder.endTime,f.adOrder.comments);showMsg(e,function(){var h=c.parent().parent().find("td:eq(0)");
h.append("<div class='loading_row'><div class='txt'>正在刷新数据&nbsp;&nbsp;&nbsp;<img src='images/loading_m.gif' /></div></div>");updateItem(c.parent().parent(),$("input[name='name']").val(),$("#datepicker_start_adorder").val(),$("#datepicker_end_adorder").val(),$("input[name='customer']").val());
$.unblockUI({onUnblock:function(){adOrderDialogId="";}});});},complete:function(e,f){},error:function(){alert("编辑失败!");}});}function updateItem(e,b,c,a,d){setTimeout(function(){$(".loading_row").remove();
e.find(".tb_name").html(b);e.find(".tb_startTime").html(c);e.find(".tb_endTime").html(a);e.find(".tb_customer").html(d==""?"-":d);},1000);}