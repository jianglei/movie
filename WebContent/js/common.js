...
function ingetCookie(){
	...
	if( history_html==null){
		document.getElementById("histroydrop_con").innerHTML = "<div id=\"view_history\" class=\"view_history\"><center>您的观看历史为空2。<\/center><\/div>";
	}else{
		...
		for(var kfi=0; kfi<(sarray.length-1); kfi++){
			...
			if(carray[0]){
				tarray = tarray + "<dd><span onclick=\"playClose('"+carray[0]+"')\"></span><a class=\"links\" href=\""+carray[2]+"\">"+carray[1]+"</a><span class=\"rplay\"><a href=\""+carray[2]+"\" >继续观看<\/a></span></dd>";
			}
		}
		if(tarray.length<1){
			document.getElementById("histroydrop_con").innerHTML = "<div id=\"view_history\" class=\"view_history\"><center>您的观看历史为空3。<\/center><\/div>";
		}else{
			document.getElementById("histroydrop_con").innerHTML = "<dl class=\"history_recommend\" id=\"history_recommend\">"+tarray+"</dl>";
		}
	}
};
