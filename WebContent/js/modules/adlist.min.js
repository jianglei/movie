...
var f='<tr class="expend_panel"><td class="tb_inner" colspan="8" ><div class="tb_inner_wrap">加载中！</div></td></tr>';c.after(f);}},success:function(f,g){loginValidate(f);
if(f[d.param.resultBody]){d.tmplList(f[d.param.resultBody]);}},complete:function(f,g){},error:function(){$(d.param.listContainer).text("加载失败！");}});},showPrior:function(a,b){var c=this;
$(".level .pnl").hide();var d=$("#"+a+".level .pnl");if(d.next("span").hasClass("i1")){d.find("li").eq(0).find("input:radio").trigger("click");}else{if(d.next("span").hasClass("i2")){d.find("li").eq(1).find("input:radio").trigger("click");
}else{d.find("li").eq(2).find("input:radio").trigger("click");}}d.show();d.find(".btn_cancel").unbind("click").click(function(){$(".level .pnl").hide();
});d.find(".btn_save").unbind("click").click(function(){c.updatePrior("",b,"/ad/prior?prior="+$("#prior_input_"+b).val()+"&id="+b);});},updatePrior:function(b,a,c){$.ajax({type:"GET",url:c,data:"",success:function(d){loginValidate(d);
$("#td_level_"+a+".level .pnl").hide();$(".prior_img_"+a).removeClass("i1 i2 i3");$(".prior_img_"+a).addClass("i"+$("#prior_input_"+a).val());}});}};