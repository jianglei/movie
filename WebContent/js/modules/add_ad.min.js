...
},changeFormStyle:function(){var c=this,d={android:{banner:{size:"202x55"},wap:{size:{standard:"202x55",image:"424x380"}},embed:{size:"202x55"},bigimage:{size:"800x250"},custom:{size:"202x55"},text:{size:"-"}},iOS:{banner:{size:"320x50"},wap:{size:{standard:"320x50",image:"480x320"}},embed:{size:"320x50"},bigimage:{size:"640x320"},custom:{size:"320x50"},text:{size:"-"}}},b=$("input[name='displayType']").val();
...
}$("#choose_ad_item").html(c);}else{$("#choose_ad_item .loadingStatus").text("没有匹配广告位!");}},showMsg:function(d,k,f,h,b){var g=this;g.initMsgAd1(d,f,h,b);
var e=$(".msg_ad"),l=$(window).height(),c=$(document).width(),m=null,j=null;$.blockUI({css:{color:"#cccccc",border:"0",width:"818px",left:c/2-(e.width()/2),top:l/2-(e.height()/2),background:"none",padding:"0px"},message:e});
$("input[name='adname']").blur().focus();$(".close").unbind("click").click(function(){$.unblockUI({onUnblock:function(){g.adDialogId="";}});});$(".btn_use_pastad").unbind("click").click(function(){m=g.getAD1();
var n="";if(m!=null){$.ajax({type:"get",url:"/ad/add2",data:{platform:g.platform,landingSize:g.landingSize},dataType:"json",beforeSend:function(o){var p=$("#adListForUseContainer");
$(".msg_ad").block({message:p,css:{border:0,background:"none",height:"430px",width:"680px",},onBlock:function(){g.adSlotExistDialogId=getToken();n=g.adSlotExistDialogId;
}});},success:function(o,p){if(n!=g.adSlotExistDialogId){return;}g.initExistAdList(o);},complete:function(o,p){},error:function(){$(".loadingStatus").text("请求失败!");
}});}});$(".btn_continue_ad").unbind("click").click(function(){m=g.getAD1();var n=g.adDialogId;if(m){if(!g.stepTwoRequest){$(".step1").hide();$(".step2").show();
g.adjustMsg();return;}if($("input[name='adid']").val()==""){g.initMsgAd2({});}else{$.ajax({type:"get",url:"/ad/edit2/"+$("input[name='adid']").val(),data:{rnd:Math.random()},beforeSend:function(o){},success:function(o,p){if(n!=g.adDialogId){return;
}g.initMsgAd2(o);},complete:function(o,p){},error:function(){alert("请求失败");}});}g.adjustMsg();}});function a(){j=g.getAD2();if(j){$(this).unbind("click");
if($("input[name='adid']").val()!==""){j.id=g.curId;}j.rnd=Math.random();var o=$.extend(m,j);var n="/ad/save";$.ajax({type:"get",url:n,dataType:"json",data:o,beforeSend:function(p){},success:function(p,q){if(p.status=="ok"){k();
}else{alert("保存失败!");}},complete:function(p,q){},error:function(){alert("保存失败!");$(".btn_save_ad").unbind("click").click(function(){a.apply(this);});}});
}}$(".btn_save_ad").unbind("click").click(function(){a.apply(this);});},adjustMsg:function(){$("body>.blockMsg").stop(true).animate({top:$(window).height()/2-($(".msg_ad").height()/2)+"px"},300);
