...
});$("#adListForUseContainer .btns .btn_save_ad2").click(function(){var b=$('#choose_ad_item input[name="record_ch"]:checked');if(b.length>0){$.ajax({type:"get",url:"/ad/edit2/"+b.val(),data:{rnd:Math.random()},beforeSend:function(c){},success:function(c,d){loginValidate(c);
a.initMsgAd2(c);},complete:function(c,d){},error:function(){alert("请求失败");}});}else{alert("请先选中一个已有广告!");}});$(".ui_radio_net").click(function(){$(this).toggleClass("ui_radio_checked");
...
a.changeFormStyle();}});$("#datepicker_ad_start").datetimepicker({dateFormat:"yy-mm-dd",timeFormat:"hh:mm",changeYear:true,changeMonth:true,monthNamesShort:["一月","二月","三月","四月","五月","六月","七月","八月","九月","十月","十一月","十二月"],dayNames:["星期日","星期一","星期二","星期三","星期四","星期五","星期六"],dayNamesMin:["日","一","二","三","四","五","六"],timeText:"时间",hourText:"时",minuteText:"分",currentText:"现在",closeText:"确定",maxDate:"+1y",onClose:function(c,b){$(this).blur();
...
$("input[name='adid']").val("");$.ajax({type:"get",url:"/ad/add1",dataType:"json",data:"rnd="+Math.random(),success:function(f,g){loginValidate(f);if(c!=a.adDialogId){return;
...
}if(d.fixedRank){$("#fixedRank").val(d.fixedRank);}else{$("#fixedRank").val("");}},initMsgAd2:function(e){var f=this;$("#loadingStatus").remove();uploadFileWait=false;
$(".msg_ad").unblock({onUnblock:function(){f.adSlotExistDialogId="";}});$(".step1").hide();$(".step2").show();adjustMsg();$(".err").remove();$("input[name='appname']").val("");
...
$('input[name="textFont"]').val("");$(".ui_select_textFont span.text").text("请选择字体");$('input[name="textSize"]').val(f.textSizeAdSlot!==""?f.textSizeAdSlot:"");
...
if(f.entryType=="push"&&b=="4"){c=[false,false,true];}else{a=g[f.entryType].adType;c=g[f.entryType].adStyle;}$(".ui_radio_form").each(function(k,l){if(!c[k]){$(l).addClass("ui_radio_disabled");
...
if(d.entryType=="wap"||(d.entryType=="push"&&c!="4")){if(b=="image"){var a=e[d.platform][d.entryType][b][d.template];$(".ui_select_landingSize span.text").text(a).attr("title",a);
...
}});},success:function(m,n){loginValidate(m);if(l!=f.adSlotExistDialogId){return;}f.initExistAdList(m);},complete:function(m,n){},error:function(){$(".loadingStatus").text("请求失败!");
}});}});$(".btn_continue_ad").unbind("click").click(function(){k=f.getAD1();var l=f.adDialogId;if(k){if(!f.stepTwoRequest){$(".step1").hide();$(".step2").show();
adjustMsg();return;}if($("input[name='adid']").val()===""){f.initMsgAd2({});}else{$.ajax({type:"get",url:"/ad/edit2/"+$("input[name='adid']").val(),data:{rnd:Math.random()},beforeSend:function(m){},success:function(m,n){loginValidate(m);
if(l!=f.adDialogId){return;}f.initMsgAd2(m);},complete:function(m,n){},error:function(){alert("请求失败");}});}adjustMsg();}});function a(){h=f.getAD2();if(h){$(this).unbind("click");
if($("input[name='adid']").val()!==""){h.id=f.curId;}h.rnd=Math.random();var m=$.extend(k,h);var l="/ad/save";$.ajax({type:"get",url:l,dataType:"json",data:m,beforeSend:function(n){},success:function(n,o){loginValidate(n);
if(n.status=="ok"){j();}else{alert("保存失败!");$(".btn_save_ad").unbind("click").click(function(){a.apply(this);});}},complete:function(n,o){},error:function(){$(".btn_save_ad").unbind("click").click(function(){a.apply(this);
...
a.stepTwoRequest=true;a.adDialogId=getToken();$("input[name='adid']").val(e);var d=$(b.target),c=a.adDialogId;$.ajax({type:"get",url:"/ad/edit1/"+e,dataType:"json",data:"rnd="+Math.random(),beforeSend:function(f){},success:function(f,h){loginValidate(f);
if(c!=a.adDialogId){return;}a.adSlotPage=f.adSlotPage.result;a.adOrderPage=f.adOrderPage.result;var g=[];for(i=0;i<f.adHolderList.length;i++){g.push(f.adHolderList[i].adSlotId);
}f.ad.adSlotId=g;a.showMsg(f.ad,function(){$.unblockUI({onUnblock:function(){a.adDialogId="";a.updateItem(d);$("body").css({"overflow-y":"auto"});}});},-1,"",b);
},complete:function(f,g){},error:function(){}});},getDataFromiTunes:function(a){$.getJSON("http://itunes.apple.com/lookup?jsoncallback=?",{id:a.replace(/.*id(\d{9})/g,"$1")},function(b){alert(b);
