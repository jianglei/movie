...
d.isTextLandingType=false;var b=[true,true],a=[true,true,true];b=e[d.entryType].adType;a=e[d.entryType].adStyle;$(".ui_radio_form").each(function(j,k){if(!a[j]){$(k).addClass("ui_radio_disabled");
}});$(".ui_radio_type").each(function(j,k){if(!b[j]){$(k).addClass("ui_radio_disabled");}});if(f.adContent){$("#adLandingType").val(f.adContent.landingType);
(function(){var m={web:0,app:1},k={standard:0,image:1,text:2};var l=$(".tb_form").find(".ui_radio_type").eq(m[f.adContent.contentType]);var j=$(".tb_form").find(".ui_radio_form").eq(k[f.adContent.displayType]);
d.isTextLandingType=f.adContent.displayType=="text";l.addClass("ui_radio_checked");$("input[name='contentType']").val(f.adContent.contentType);j.addClass("ui_radio_checked");
...
}}else{var g=$(".ui_radio_type").not(".ui_radio_disabled");var c=g.length==1?g.eq(0):g.eq(1);var h=$(".ui_radio_form").not(".ui_radio_disabled").eq(0);
$("input[name='contentType']").val(c.attr("title"));$("input[name='displayType']").val(h.attr("style_type"));c.trigger("click");h.trigger("click");}if(d.isTextLandingType){d.textPreviewChange();
...
}$("#choose_ad_item").html(c);}else{$("#choose_ad_item .loadingStatus").text("没有匹配广告位!");}},showMsg:function(c,j,e,g,a){var f=this;f.initMsgAd1(c,e,g,a);
...
}else{alert("保存失败!");}},complete:function(o,p){},error:function(){alert("保存失败!");}});}});},adjustMsg:function(){$("body>.blockMsg").stop(true).animate({top:$(window).height()/2-($(".msg_ad").height()/2)+"px"},300);
